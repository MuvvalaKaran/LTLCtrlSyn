...
class TransSysToPolytope(object):
    ...
    # need to work from here on. Also need to understand on whats going on in this code.
        ...
        for i in range(pow(2,Nh)):
            ...
            H_A = np.append(H_A, A[len(A)-1], 0)
            H_B = np.diag(np.outer(np.array(signs[i]), Bin))
            H_B = np.append(H_B[:, None], b[len(b)-1], 0)
        ...
        for i in range(0,sp_no):
            ...
            for j in range(0,N_p):
                ...
                if A[j].ndim == 1:
                    ...
                    tmp = signs[0:A[j].shape[0]]
                    # check if tmp is all zeroes or not
                    if not np.any(tmp):
                        ap_obs.append(j)
                    signs = np.delete(signs, np.s_[0:A[j].shape[0]])
...
#     tmp = TransSysToPolytope(A, B)
#     tmp.transystopolytope()