        ...
        for i in range(0,len(Tp_Q)):
            ...
            # for all vertices of si check feasibility by checking non-emptiness of optimisation set (imposed by restrictions)
                for l in range(v_no):
                    ...
                    if rank_method:
                        H_rep_A = np.vstack((U_A, np.matmul(-1*F_n[ex_f, :], D_B)))
                        if len(in_f) == 1:
                            H_rep_A = np.vstack((H_rep_A, np.matmul(F_n[in_f[0], :], D_B)))
                        else:
                            H_rep_A = np.vstack((H_rep_A, np.matmul(F_n[in_f[0]:in_f[1], :], D_B)))

                        abc = np.matmul(D_A, tranfosefor1dvector(V[l, :], 1)) + D_b
                        H_rep_B = np.vstack((-1 * U_b, (np.matmul(F_n[ex_f, :], abc) - prec)))
                        abc = np.matmul(D_A, tranfosefor1dvector(V[l, :], 1)) + D_b
                        if len(in_f) == 1:
                    ...
                    if linprog_method:
                        if len(in_f) == 1:
                            abcd1 = np.vstack((np.matmul(-1*F_n[ex_f, :], D_B), np.matmul(F_n[in_f[0], :], D_B)))
                        else:
                            abcd1 = np.vstack((np.matmul(-1 * F_n[ex_f, :], D_B), np.matmul(F_n[in_f[0]:in_f[1], :], D_B)))
            ...
            for m in range(0, v_no):
                ...
                if rank_method:
                    if len(in_f) == 1:
                        H_rep_A = np.vstack((U_A, np.matmul(F_n[in_f[0], :], D_B)))
                        tmp = np.matmul(D_A, tranfosefor1dvector(V[m, :],1)) + D_b
                        H_rep_B = np.vstack((-1*U_b, np.matmul(-1 * F_n[in_f[0], :], tmp)))
                    else:
                        H_rep_A = np.vstack((U_A, np.matmul(F_n[in_f[0]:in_f[1], :], D_B)))
                        tmp = np.matmul(D_A, tranfosefor1dvector(V[m, :], 1)) + D_b
                        H_rep_B = np.vstack((-1 * U_b, np.matmul(-1 * F_n[in_f[0]:in_f[1], :], tmp)))
                    p = pc.Polytope(H_rep_A, H_rep_B)
                    try:
                        V_rep = pc.extreme(p)
